...
public class Parser {
	...
	public boolean isSemicolon(Token t) {
		return (TokenType.CHAR == t.getType() && ";".equals(t.getSVal()));
	}
	...
	public boolean expect(TokenType expected, Token got) {
		if (expected == got.getType()) {
			...
			System.err.println("Expected " + expected + ", but found " + got.getSVal());
			return false;
		}
	}

	public boolean expect(String expected, Token got) {
		if ((TokenType.CHAR == got.getType()) && (expected.equals(got.getSVal()))) {
			return true;
		} else {
			System.err.println("Expected " + expected + ", but found " + got.getSVal());
			return false;
	...
	public boolean expectLogicTest(Program p, Token t, Lexer l) {
		if (TokenType.TEST == t.getType()) {
			...
			System.err.println("Expected logic test, but found " + t.getSVal());
			return false;
		}
	}
	...
	public boolean expectEndExec(Program p, Token t, Lexer l) throws java.io.IOException {
		if (TokenType.ENDEXEC == t.getType()) {
			...
			System.err.println("Expected END-OF-EXECUTION, but found " + t.getSVal());
			return false;
		}
	}
	...
	public boolean expectEndProg(Program p, Token t, Lexer l) throws java.io.IOException {
		if (TokenType.ENDPROG == t.getType()) {
			...
			System.err.println("Expected END-OF-PROGRAM, but found " + t.getSVal());
			return false;
		}
	}
	...
	public boolean expectEndOfFile(Program p, Token t, Lexer l) {
		if (TokenType.EOF == t.getType()) {
			...
			System.err.println("Expected END-OF-FILE, but found " + t.getSVal());
			return false;
		}
	}
	...
	public boolean expectStmt(Program p, Token t, Lexer l) throws java.io.IOException {
		if (TokenType.BEGIN == t.getType()) {
			return (expectStmtList(p, popToken(l), l) && expectEnd(popToken(l)));
			...
			if (expectNumber(p, popToken(l), l)) {
				...
				p.install();
				if (expectTimes(popToken(l)) && expectStmt(p, popToken(l), l)) {
					p.install(Instruction.RETURN);
			...
			if (expectLogicTest(p, popToken(l), l)) {
				...
				int jumploc = p.getProgp();
				p.install();
				if (expectThen(popToken(l)) && expectStmt(p, popToken(l), l)) {
					p.install(jumploc, p.getProgp());
			...
			if (expectLogicTest(p, popToken(l), l)) {
				p.install();
				p.install();
				if (expectDo(popToken(l)) && expectStmt(p, popToken(l), l)) {
			...
			System.err.println("Unexpected token " + t.getSVal() + " in expectStmt");
			return false;
		}
	}
	...
	public boolean expectMain(Program p, Token t, Lexer l) throws java.io.IOException {
		return (expectStmtList(p, t, l) && (expectEndExec(p, popToken(l), l)));
	}
	...
	public boolean expectBegExec(Program p, Token t, Lexer l) throws java.io.IOException {
		if (TokenType.BEGEXEC == t.getType()) {
			...
			System.err.println("Expected BEGINNING-OF-EXECUTION, but found " + t.getSVal());
			return false;
		}
	}
	...
	public boolean expectSemicolon1(Program p, Token t, Lexer l) throws java.io.IOException {
		return expect(";", t);
	}
	...
	public boolean expectDefInst(Program p, Token t, Lexer l) throws java.io.IOException {
		if (TokenType.DEFINST == t.getType()) {
			...
			if (expectAs(popToken(l)) && expectStmt(p, popToken(l), l) && expectSemicolon1(p, popToken(l), l)) {
				p.install(Instruction.RETURN);
	...
	public boolean expectProgram(Program p, Token t, Lexer l) throws java.io.IOException {
		if (TokenType.BEGPROG == t.getType()) {
			...
			System.err.println("Expected BEGINNING-OF-PROGRAM, but found " + t.getSVal());
			return false;
		}
	}
