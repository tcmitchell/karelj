...
public class WorldPanel extends JPanel implements ComponentListener, PropertyChangeListener {
	...
	public void setRobot(Robot r) {
		if (fRobot != null) {
			fRobot.removePropertyChangeListener(this);
		}

		fRobot = r;
		if (fWorld != null) {
			fRobot.setWorld(fWorld);
		}
		...
		for (int a = 1; a <= getWorldAvenues(); a++) {
			for (int s = 1; s <= getWorldStreets(); s++) {
				if (fWorld.checkBeeper(a, s)) {
					...
					g.fillOval(pt.x - x_offset, pt.y - y_offset, beeper_width, beeper_height);
					g.setColor(Color.black);
					g.drawOval(pt.x - x_offset, pt.y - y_offset, beeper_width, beeper_height);
					g.drawOval(pt.x - x_offset, pt.y - y_offset, beeper_width + 1, beeper_height + 1);
				}
			}
		}
	}

	protected void drawWideVerticalLine(Graphics g, int x1, int y1, int x2, int y2) {
		g.drawLine(x1, y1, x2, y2);
		g.drawLine(x1 + 1, y1, x2 + 1, y2);
		g.drawLine(x1 - 1, y1, x2 - 1, y2);
	}

	protected void drawWideHorizontalLine(Graphics g, int x1, int y1, int x2, int y2) {
		g.drawLine(x1, y1, x2, y2);
	...
	protected void drawWalls(Graphics g) {
		...
		drawWideVerticalLine(g, pt1.x - half_cell_width, pt2.y - half_cell_height, pt1.x - half_cell_width,
				pt1.y + half_cell_height);

		// Draw the south wall
		pt2 = worldToScreen(getWorldAvenues(), 1);
		drawWideHorizontalLine(g, pt1.x - half_cell_width, pt1.y + half_cell_height, pt2.x + half_cell_width,
				pt1.y + half_cell_height);
		...
		for (int s = 1; s <= getWorldStreets(); s++) {
			for (int a = 1; a <= getWorldAvenues(); a++) {
				if (fWorld.checkNSWall(a, s)) {
					...
					drawWideVerticalLine(g, pt1.x + half_cell_width, pt1.y - half_cell_height, pt1.x + half_cell_width,
							pt1.y + half_cell_height);
				}
				if (fWorld.checkEWWall(a, s)) {
					pt1 = worldToScreen(a, s);
					drawWideHorizontalLine(g, pt1.x - half_cell_width, pt1.y - half_cell_height,
							pt1.x + half_cell_width, pt1.y - half_cell_height);
				}
			}
		}
	}
	...
	protected void drawGrid(Graphics g) {
		...
		for (int s = 1; s <= getWorldStreets(); s++) {
			...
			g.drawLine(pt1.x - half_cell_width, pt1.y, pt2.x + half_cell_width, pt2.y);
		}
		...
		for (int a = 1; a <= getWorldAvenues(); a++) {
			...
			g.drawLine(pt1.x, pt1.y + half_cell_height, pt2.x, pt2.y - half_cell_height);
		}
	}
