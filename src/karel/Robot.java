...
public class Robot {
    ...
    static {
        try {
            String ms = System.getProperty("karel.sleepInterval", defaultSleepInterval.toString());
    ...
    /**
     * For backward compatibility. Deprecate soon. Used in Parser test code, and
     * Trace test code.
     */
    ...
    public void move() throws KarelOutOfBoundsException, KarelCrashedException {
        switch (direction) {
        case NORTH:
            if (!world.isInBounds(getAvenue(), getStreet() + 1)) {
                throw new KarelOutOfBoundsException();
            } else if (world.checkEWWall(getAvenue(), getStreet())) {
